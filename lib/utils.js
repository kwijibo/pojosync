...
var Utils = {
  ...
  copyResource: function(resource, copied, copies){
    var clone = {};
    copied = copied || [];
    copies = copies || [];
    ...
    for(var p in resource){
      ...
      if(typeof(next=='object')){
        var i = copied.indexOf(next);
        if(i>-1) clone[p]=copies[i];
        else clone[p]=Utils.copyResource(next,copied,copies);
      } else {
        clone[p]=next;
      }
    }
  ...
  matchesFilter: function(resource, filter){
    for(var k in filter){
  ...
  generateID: function(){
    ...
    return id + String(now.getTime() - epoch.getTime());
  }
