...
Store.prototype.indexResource = function(resource){
  var self= this;
  if(Array.isArray(resource)){
    for(var i = 0; i < resource.length;i++){
      self.indexResource(resource[i]);
    }
    return resource;
...
Store.prototype.flatten = function(resource, index){
  ...
  } else if(typeof resource=='object' && !index[resource.id]){
  ...
  if(typeof(resource)=='string' && resource[0]=='@'){
    ...
    for(var p in resource){
      switch(typeof(resource[p])){
        case 'object': 
          self.unflatten(resource[p]);
          break;
         default:
          resource[p] = self.unflatten(resource[p]);
          break;
      }
    }
  } else {
